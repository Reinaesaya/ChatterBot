...
class ConvoStarterTrainer(Trainer):

	def __init__(self, storage, **kwargs):
		super(ConvoStarterTrainer, self).__init__(storage, **kwargs)

	def train(self, convostarter_file):
		trainer = ListTrainer(self.storage)
		lines = []

		with open(convostarter_file,'r') as f:
			lines = f.readlines()
		...
		for l in lines:
			...
			if parse:
				...
				if istopic:
					if len(l.strip()) == 0:
						continue
					topics = [x.strip().lower() for x in l.split(",") if len(x.strip())>0]
					istopic = False
					print(topics)
				else:
					#print([topic, l.strip()])
